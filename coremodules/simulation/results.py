...
class EdgeresultsEvolution(Edgeresults):
    ...
    def adapt_intervals(self, time_start_recording, n_interval, time_interval, ids_edge = None):
        ...
        for attrname, attrconf in self.get_group_attrs('results').iteritems():
    ...
    def reset_intervals(self, time_start_recording, n_interval, time_interval, is_fill = False, ids_edge = None):
        ...
        if (self.time_start_recording.get_value() != time_start_recording) | (self.n_interval.get_value() != n_interval) | (self.time_interval.get_value() != time_interval):
            ...
            attrnames = self.get_group_attrs('results').keys()
    ...
    def import_sumoxml(self, filepath,  attrconfigs = None, time_start_recording = 0,  time_end_recording = np.inf):    
        ...
        print '  len(ids_edge)',len(ids_edge),self.ids_edge.get_linktab()
    ...
    def import_sumoxml_slow(self, filepath,   attrconfigs):    
        ...
        for ind_int in xrange(self.n_interval.get_value()):
            #print '='*70
            time_start_recording, time_end_recording = self.get_timeinterval(ind_int)
            ...
            for attrconfig in attrconfigs: 
                ...
                if type(default) in (np.int32, np.int64,types.IntType, types.LongType):
                    conversion = 'i'#int
                    #values_attr = np.zeros(n,dtype = np.int32)
                elif type(default) in (np.float32, np.float64, types.FloatType,types.ComplexType):
                    conversion = 'f'#float
                    #values_attr = np.zeros(n,dtype = np.float32)
                else:
                    conversion = 's'#str
    ...
    def __init__(self, element, counter, ids_sumo, time_start_recording, time_end_recording,  attrsconfigs ):
        ...
        for  attrconfig in attrsconfigs:
            ...
            if attrtype in am.TYPES_INT:
                ...
                self._values[attrconfig.attrname]= np.zeros((n_edges, counter.n_interval), dtype = np.object)
